...
jest.mock('./email-orchestration.service', () => ({ acceptEmailIntent: jest.fn().mockResolvedValue({}) }));
...
describe('ingestVerificationSubmission', () => {
  ...
  it('creates the next submission version with a three-day SLA', async () => {
    ...
    (VerificationSubmissionModel.create as jest.Mock).mockImplementation(async (value) => ({ ...value, _id: 'submission-3' }));
    ...
    expect(acceptEmailIntent).toHaveBeenCalledWith(expect.objectContaining({ templateKey: 'VERIFICATION_UPDATE', to: 'office@example.test' }));
  });
