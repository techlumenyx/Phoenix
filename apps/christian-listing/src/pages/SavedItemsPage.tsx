...
export default function SavedItemsPage() {
  ...
  return <main className="mx-auto max-w-7xl px-5 pb-12 pt-28 md:px-10">
    ...
    {(tab === 'ALL' || tab === 'EVENTS') && events.length > 0 && <SavedSection title="Events">{events.map(({ event }) => <SavedCard key={event.id} onRemove={() => remove('EVENTS', event.id)}><EventCard title={event.title} description={event.description} badge={event.category.replaceAll('_',' ')} date={new Date(event.date).toLocaleDateString()} location={event.region} invites={`${event.rsvpCount} attending`} verified={event.hosts.some((host) => host.isVerified)} imageSrc={event.imageUrls[0]} href={`/events/${event.id}`} /></SavedCard>)}</SavedSection>}
