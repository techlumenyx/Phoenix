...
export default function MarketplaceDetailsPage() {
  ...
  return (
    <main className="min-h-screen bg-white px-5 pb-8 pt-28 text-[#0d1727] md:px-10 lg:px-16">
...
function PageMessage({ title, detail }: { title: string; detail?: string }) { return <main className="flex min-h-[65vh] items-center justify-center px-6 pt-20 text-center"><div><h1 className="font-serif text-3xl font-bold">{title}</h1>{detail && <p className="mt-3 text-sm text-gray-500">{detail}</p>}<Link to="/marketplace" className="mt-6 inline-block rounded-full bg-[#004b3d] px-5 py-2.5 text-sm text-white">Browse marketplace</Link></div></main>; }
