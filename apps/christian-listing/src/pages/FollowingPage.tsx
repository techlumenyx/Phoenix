...
export default function FollowingPage() {
  ...
  return <main className="mx-auto max-w-7xl px-5 pb-12 pt-28 md:px-10"><p className="text-sm text-gray-500"><Link to="/dashboard">Dashboard</Link> / Following</p><h1 className="mt-2 font-serif text-4xl font-bold">Following</h1><p className="mt-2 text-sm text-gray-500">Latest activity from organisations you care about.</p>
    {loading && !data && <p className="py-20 text-center text-sm text-gray-500">Loading your community...</p>}{error && <p className="py-20 text-center text-sm text-red-600">We couldn’t load the organisations you follow. Please try again.</p>}
    ...
    {organisations.length > 0 && <section className="mt-10"><h2 className="font-serif text-2xl font-bold">Your organisations</h2><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{organisations.map((org) => <Link to={`/organisations/${org.id}`} key={org.id} className="flex gap-4 rounded-2xl border border-gray-200 bg-white p-5 hover:shadow-md"><NameAvatar name={org.name} src={org.logoUrl} className="h-14 w-14 rounded-full text-xl" /><div><h3 className="font-semibold">{org.name} {org.isVerified && <span className="text-green-600">✓</span>}</h3><p className="mt-1 text-xs text-gray-500">{org.region || 'Global community'} · {org.followerCount} followers</p><p className="mt-2 line-clamp-2 text-xs leading-5 text-gray-500">{org.description}</p></div></Link>)}</div></section>}
    <Feed title="Recent events">{events.slice(0, 8).map((event) => <EventCard key={event.id} title={event.title} description={event.description} badge={event.category} date={new Date(event.date).toLocaleDateString()} location={event.region} invites={`${event.rsvpCount} attending`} verified={event.org.isVerified} imageSrc={event.imageUrls[0]} href={`/events/${event.id}`}/>)}</Feed>
